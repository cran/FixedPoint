...
The implementation of the Newton method in this package uses this formula to predict the fixed point given two previous iterates.[^1] This method is designed for use with scalar functions. If it is used with higher dimensional functions that take and return vectors then it will be used elementwise. For vector elements for which $g(x_i) = g(x_{i-1})$ and the derivative is zero **FixedPoint** will substitute in the corresponding value from a simple iterate.
...
For vector elements for which $x_{i+2} - 2x_{i+1} + x_i = 0$  and there is division by zero **FixedPoint** will substitute in the corresponding value from a simple iterate.
...
[^5]: When these complications arise the Newton method and the Aitken method will revert to using the simple method to create
a new guess for the fixed point. A warning will be emitted to inform the user of this. 
