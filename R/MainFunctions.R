...
#' @param Inputs This can be either a vector of values that is an initial guess for a fixed point or it can be an N x A matrix of previous inputs for which corresponding
#'  outputs are available. In this case N is the dimensionality of the fixed point vector you are seeking (Hence each column is a matrix that is input to the "Function")
#'  and A is the number of previous Inputs/Outputs that are being provided to the fixed point. Where a matrix is input, a corresponding outputs must be provided or the last
#'  column of the outputs matrix is taken as a startpoint guess and the rest of the inputs and output matrices are discarded.
#' @param Outputs (Optional) This is a matrix of the Function values for each column of the input. It must be provided so that column k of the outputs matrix is equal to
#'  Function(Column k of inputs matrix).
#' @param Method This is the fixed point method to be used. It can be "Anderson", "Simple", "Aitken", "Newton", "MPE", "RRE", "VEA" or "SEA". See vignette and references
#'  to see explanations of these methods.
#' @param ConvergenceMetric This is a function that takes in a vector of residuals from one iterate of the function (defined as f(x) - x for vector x and function f) and
#'  returns a scalar. This scalar should be low when convergence is close to being achieved. By default this is the maximum residual by absolute value (the sup norm in the
#'  space of residuals).
#' @param ConvergenceMetricThreshold This is the threshold for terminating the algorithm. The algorithm will terminate when the scalar that ConvergenceMetric returns is less
#'  than ConvergenceMetricThreshold. This can be set to a negative number in which case the algorithm will run until MaxIter is hit or an error occurs (Note that an error is
#'  likely in trying to use any method other than "Simple" when a fixed point is already found).
#' @param MaxIter This is the maximum number of iterates that will be undertaken.
#' @param MaxM This is the maximum number of saved iterates that are used in the Anderson algorithm. It has no effect if another method is chosen. Note that the number of
#'  previous iterates that will actually be used is the minimum of MaxIter, the dimensionality of the function's vector and the number of inputs that have been tried to
#'  previously (the width of the Outputs matrix at each given stage of the algorithm). If PrintReports = TRUE, the number of previous iterates actually used is reported as
#'  the algorithm is running.
#' @param ExtrapolationPeriod This is the number of simple iterates to perform before extrapolating. This is used for the MPE, RRE, VEA and SEA methods and has no effect
#' if another method is chosen Where an epsilon algorithm is used this should be a multiple of 2, ie (4,6,8,etc).
#' @param Dampening This is the dampening parameter. By default it is 1 which means no dampening takes place. It can also be less than 1 (indicating dampening) or more
#' than 1 (indicating extrapolation).
#' @param PrintReports This is a boolean describing whether to print ongoing ConvergenceMetric values for each iterate.
#' @param ReportingSigFig This is the number of significant figures that will be used in printing the convergence values to the console (only if PrintReports is TRUE).
#' @param ConditionNumberThreshold This is a threshold for what condition number is acceptable for solving the least squares problem for the Anderson Method. If the
#' condition number is larger than this threshold then fewer previous iterates will be used in solving the problem. This has no effect unless the "Anderson" method is
#' used.
#' @param Plot This determines whether a plot should be drawn for every iterate. It can be "NoPlot", "ConvergenceFig" or "ChangePerIterate". By default it is
#' "NoPlot" and no plot is drawn. If it is "ConvergenceFig" then a plot is shown with iterates on the x axis and convergence (as defined by the ConvergenceMetric) is on
#' the y axis. If it is "ChangePerIterate" then there is the index of the array value on the x axis and the value of the array value on the y axis. The previous iterate
#' is also shown so the change per iterate can be visualised.
#' @param ConvergenceFigLags This only affects anything if Plot == "ConvergenceFig". This gives how many previous iterates should be shown on the x axis. By default it
#' is 5. To see them all set it to a high number.
#' @param ChangePerIteratexaxis This only affects anything if Plot == "ChangePerIterate". Sometimes there is a more appropriate xaxis value to use than (the default)
#' value index for this figure. For instance in the consumption smoothing problem in the vignette every value is a value function value at a given budget level.
#' In this case the budget levels could be used for this xaxis.
#' @return A list containing the FixedPoint, the Inputs and corresponding Outputs, and convergence values (which are computed under the "ConvergenceMetric").
...
#' # But we can continue solving for this fixed point but now switching to the Newton method.
...
FixedPoint = function(Function, Inputs, Outputs = c(), Method = c("Anderson", "Simple", "Aitken", "Newton", "MPE", "RRE", "VEA", "SEA") , ConvergenceMetric  = function(Resids){max(abs(Resids))} , ConvergenceMetricThreshold = 1e-10, MaxIter = 1e3, MaxM = 10, ExtrapolationPeriod = 7, Dampening = 1, PrintReports = FALSE, ReportingSigFig = 5, ConditionNumberThreshold = 1e3, Plot = c("NoPlot", "ConvergenceFig", "ChangePerIterate"), ConvergenceFigLags = 5, ChangePerIteratexaxis = c()){
  ...
  # There are several fixed point methods that are available. Anderson, Simple and Newton.
  Method = Method[1]
  Plot   = Plot[1]
  ...
  if (is.null(Outputs)){
    if (dim(Inputs)[2] > 1.5){
      warning("If you do not give outputs to the function then you can only give one vector of inputs. As you have input a matrix of input values everything but the last column has been discarded.\n")
      Inputs = Inputs[,dim(Inputs)[2]]
      }
  } else {
    if (sum(dim(Inputs) != dim(Outputs)) > 0.5) {
      warning("If you input a matrix of outputs as well as a matrix of inputs then inputs and outputs must be the same shape. As they differ in this case the last column of the inputs matrix has been
  ...
  if (is.null(Outputs)){
    ...
    if (class(NewOutputVector) == "try-error"){return(list(Inputs = Inputs, Outputs = NA, Convergence = NA, FixedPoint = NA, Finish = "Could not execute function with input vector"))}
    if (sum(is.na(NewOutputVector))){return(list(Inputs = Inputs, Outputs = NA, Convergence = NA, FixedPoint = NA, Finish = "New output vector contains NAs", NewInputVector = Inputs, NewOutputVector = NewOutputVector))}
  ...
  if (PrintReports){cat(paste0(format(" ", width = 49, justify = "right"), "Method: ", format(Method, width = 8, justify = "right")   , ". Iteration: ", format(iter, digits =  0, width = 5,scientific = FALSE),
  ...
  if (Plot == "ChangePerIterate"){ChangePerIterate(Inputs, Outputs, ConvergenceVector, FromIterate = dim(Inputs)[2], ToIterate = dim(Inputs)[2], xaxis = ChangePerIteratexaxis, secondhold = -1)}
  iter = iter + 1

  while ((Convergence > ConvergenceMetricThreshold) & (iter <= MaxIter)){
    # Generating new input and output.
    NewInputVector = try({FixedPointNewInput(Inputs = Inputs, Outputs = Outputs, Method = Method, MaxM = MaxM, SimpleStartIndex = SimpleStartIndex, ExtrapolationPeriod = ExtrapolationPeriod, Dampening = Dampening, ConditionNumberThreshold = ConditionNumberThreshold, PrintReports = PrintReports)})
    if (class(NewInputVector) == "try-error"){return(list(Inputs = Inputs, Outputs = Outputs, Convergence = ConvergenceVector, FixedPoint = NA, Finish = "Could not generate new input vector"))}
    if (sum(is.na(NewInputVector))){return(list(Inputs = Inputs, Outputs = Outputs, Convergence = ConvergenceVector, FixedPoint = NA, Finish = "New input vector contains NAs", NewInputVector = NewInputVector))}

    if (Method != "Anderson" & PrintReports){cat(paste0(format(" ", width = 49, justify = "right")))}

    NewOutputVector = try({Function(NewInputVector)})
    if (class(NewOutputVector) == "try-error"){return(list(Inputs = Inputs, Outputs = Outputs, Convergence = ConvergenceVector, FixedPoint = NA, Finish = "Could not execute function with generated vector", NewInputVector = NewInputVector))}
    if (sum(is.na(NewOutputVector))){return(list(Inputs = Inputs, Outputs = Outputs, Convergence = ConvergenceVector, FixedPoint = NA, Finish = "New output vector contains NAs", NewInputVector = NewInputVector, NewOutputVector = NewOutputVector))}
    ...
    if (PrintReports){cat(paste0("Method: ", format(Method, width = 8, justify = "right")   , ". Iteration: ", format(iter, digits =  0, width = 5,scientific = FALSE), ". Convergence: ", NicePrint(Convergence, ReportingSigFig), "\n"))}
    if (Plot == "ConvergenceFig"){ConvergenceFig(Inputs, Outputs,  Input_Convergence =  ConvergenceVector, FromIterate = max(1, dim(Inputs)[2]- ConvergenceFigLags))}
    if (Plot == "ChangePerIterate"){ChangePerIterate(Inputs, Outputs, ConvergenceVector, secondhold = -1, FromIterate = dim(Inputs)[2], ToIterate = dim(Inputs)[2], xaxis = ChangePerIteratexaxis)}
    iter  = iter + 1
  }
...
#' @export
#' @param Inputs This is an N x A matrix of previous inputs for which corresponding outputs are available. In this case N is the dimensionality of the fixed
...
#' @param SimpleStartIndex This is the index for what column in the input/output matrices did the algorithm start doing simple iterates without jumps.
#' This is used for all methods except the simple and Anderson methods where it has no effect.
#' @param ExtrapolationPeriod This is the number of simple iterates to perform before extrapolating. This is used for the MPE, RRE, VEA and SEA methods and
#' has no effect if another method is chosen.
#' @param Dampening This is the dampening parameter. By default it is 1 which means no dampening takes place. It can also be less than 1 (indicating dampening)
#' or more than 1 (indicating extrapolation).
#' @param ConditionNumberThreshold This is what threshold should be chosen to drop previous iterates if the matrix is ill conditioned. Only used in Anderson
#' acceleration.
#' @param PrintReports This is a boolean describing whether to print ongoing ConvergenceMetric values for each iterate.
...
FixedPointNewInput = function(Inputs, Outputs, Method = "Anderson", MaxM = 10, SimpleStartIndex = 1, ExtrapolationPeriod = 1, Dampening = 1, ConditionNumberThreshold = 1e3, PrintReports = FALSE ) {
  ...
  if (Method == "Anderson"){
    ...
    if (PrintReports){cat(paste0("Condition number is ", format(format(ConditionNumber, digits = 5,scientific = TRUE), width = 12,justify = "right")  ,". Used:",  format(M+1, width = 3, justify = "right")," lags. "))}
  ...
  if (Method == "Aitken"){
    if ((CompletedIters + SimpleStartIndex) %% 3 == 0){
      ...
      NewGuess[is.nan(NewGuess)] = ffx[is.nan(NewGuess)]
  ...
  if (Method == "Newton"){
    if ((CompletedIters + SimpleStartIndex) %% 2 == 1 & CompletedIters > 1){
      ...
      NewGuess[is.nan(NewGuess)] = fxk[is.nan(NewGuess)]
      return(as.vector(Dampening*NewGuess + (1-Dampening)*Outputs[,CompletedIters] ))
    } else {
...
#' @export
#' @import MASS
#' @param Iterates A matrix of inputs and outputs excluding jumps. Can be pieced together from Inputs and Outputs matrices of the FixedPoint function using the
#' PutTogetherIteratesWithoutJumps function
#' @param Method The method for polynomial extrapolation. Should be either "MPE" for minimal polynomial extrapolation or "RRE" for reduced rank extrapolation.
...
PolynomialExtrapolation = function(Iterates, Method = c("MPE", "RRE")){
  ...
  if (Method == "MPE"){
    ...
    Differences          = matrix(Iterates[,2:(TotalColumnsOfIterates-1)] - Iterates[,1:(TotalColumnsOfIterates-2)], ncol = TotalColumnsOfIterates-2)
  ...
  if (Method == "RRE"){
    ...
    Differences          = matrix(Iterates[,2:(TotalColumnsOfIterates)] - Iterates[,1:(TotalColumnsOfIterates-1)], ncol = TotalColumnsOfIterates-1)
    SecondDifferences    = matrix(Differences[,2:(TotalColumnsOfIterates-1)] - Differences[,1:(TotalColumnsOfIterates-2)], ncol = TotalColumnsOfIterates-2)
...
#' @export
#' @import MASS
#' @param Iterates A matrix representing different iterates with one iterate per column. Can be pieced together from Inputs and Outputs matrices of
#' the FixedPoint function using the PutTogetherIteratesWithoutJumps function
#' @param Method Method for epsilon extrapolation. Should be either "VEA" for the vector extrapolation algorithm or "SEA" for the scalar epsilon algorithm.
...
EpsilonExtrapolation = function(Iterates, Method = c("VEA", "SEA")){
    ...
    for (MatrixColumns in TotalColumnsOfMatrix:(2)){
      NewMatrix = PreviousMatrix + EpsilonExtrapolationVectorOfInverses(matrix(Matrix[,2:MatrixColumns] - Matrix[,1:(MatrixColumns-1)],nrow = RowsOfMatrix), Method = Method)
...
#' @export
#' @param Inputs This is an N x A matrix of previous inputs for which corresponding outputs are available. In this case N is the dimensionality of the
#' fixed point vector that is being sought (and each column is a matrix that is input to the "Function") and A is the number of previous Inputs/Outputs
...
#' @param AgreementThreshold A parameter for determining when a column in Inputs and a column in Outputs match. They are deemed to match if the sum of
#' the absolute values of the difference in the columns is less than AgreementThreshold.
...
PutTogetherIteratesWithoutJumps = function(Inputs, Outputs, AgreementThreshold = 1e-10){
  if (!(all(is.matrix(Inputs), is.matrix(Outputs)))){stop("Inputs and/or Outputs is not a matrix.")}
  ...
  return( matrix(cbind(   matrix(Inputs[,(LastBreak+1):(Dimensions[2])] , ncol = Dimensions[2] - LastBreak)   , matrix(Outputs[, Dimensions[2]], ncol = 1))   , ncol = Dimensions[2] - LastBreak + 1)       )
}
...
NicePrint = function(Number, sigfig = 5){
  CPrint = format(Number, digits =  sigfig,scientific = TRUE)
  LenCPrint = nchar(CPrint)
  CPrint = paste0(substr(CPrint,1,LenCPrint-4)  , paste0(rep(0,sigfig+5-LenCPrint),collapse = "") , substr(CPrint,LenCPrint-3,LenCPrint))
  return(CPrint)
}
